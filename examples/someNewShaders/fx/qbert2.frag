...
//f1:vert stretch:
//f2::
float f0 = mix(0.15, 0.95, fparams[0]);
float f1 = mix(0.05, 0.95, fparams[1]);
float f2 = mix(0.05, 0.95, fparams[2]);

float time = float(itime) + ftime;

float cellFactor = floor(clamp(tres.x/30., 20., 100.)/6.)*12.;
vec2 cellSize = vec2(cellFactor , cellFactor*1.5) / vec2(f0,f1);
...
void main(void) {
  ...
  vec2 cell = vec2(mod(gl_FragCoord.x, cellSize.x), mod(gl_FragCoord.y, cellSize.y));
    ...
    float bright = 1.0;
    float glitch = 0.375;//.375 orig


    //1
    if (tri(vec2(0.0,0.0), vec2(1.0,0.0), vec2(0.5,0.125), norm)) {res = vec2(0.5, -0.125); bright = light;}
    //2
    else if (tri(vec2(0.0,0.0), vec2(0.5,0.125), vec2(0.0,glitch), norm)) {res = vec2(0.0, glitch); bright = dark;}
    else if (tri(vec2(0.5,0.125), vec2(0.5,0.5), vec2(0.0,glitch), norm)) {res = vec2(0.0, glitch); bright = dark;}
    //3
    else if (tri(vec2(1.0,0.0), vec2(1.0,glitch), vec2(0.5,0.125), norm)) {res = vec2(1.0, glitch);}
    else if (tri(vec2(0.5,0.125), vec2(0.5,0.5), vec2(1.0,glitch), norm)) {res = vec2(1.0, glitch);}
    //4
    else if (tri(vec2(0.0,glitch), vec2(0.0,.625), vec2(0.5,0.5), norm)) {res = vec2(0.0, glitch); bright = light;}
    //5
    else if (tri(vec2(1.0,glitch), vec2(1.0,.625), vec2(0.5,0.5), norm)) {res = vec2(1.0, glitch); bright = light;}
    //6
    ...
    for(float i = -1.5; i<1.5; i+=1.0)
    {
        for (float j = -1.5; j<1.5; j+=1.0)
        {
            gl_FragColor += clamp(texture2D(
                tex, 
                vec2(
                    ((floor(gl_FragCoord.x/cellSize.x)+res.x)*cellSize.x+i)/tres.y,
