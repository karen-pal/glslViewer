...
void main(void) {
    ...
    float dotsize = 20.0 + sin(time) * 0.0005; // slow, larger dots

    vec2 mirrorCoord = vec2 (tres.x - gl_FragCoord.x, gl_FragCoord.y) * vec2(f0,f1);
    ...
    float inside = smoothstep (dotRad + 2.0, dotRad /f2, rad);
